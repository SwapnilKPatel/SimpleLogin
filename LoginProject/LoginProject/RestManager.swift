...
class RestManager {
    ...
    func makeRequest(toURL url: URL,prams : [String:Any] = [:],
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            guard var request = self?.prepareRequest(withURL: targetURL, httpBody: httpBody, httpMethod: httpMethod) else
            {
                completion(Results(withError: CustomError.failedToCreateRequest))
                return
            }
            request.httpBody = try! JSONSerialization.data(withJSONObject:prams, options: [])
